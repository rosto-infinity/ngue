...
<div class="min-h-screen bg-gradient-to-br from-[#0f172a] via-[#112825] to-[#0f172a] flex flex-col justify-between">
    ...
    <div class="pt-16 px-6 flex flex-col items-center">
        ...
        <div class="mb-4 animate-fade-in-down">
            <img src="{{ asset('images/logi.jpg') }}" alt="Ngue Ngan Result Logo" class="w-24 h-24 rounded-full shadow-lg border-2 border-with-500 p-1">
        </div>

        <h1 class="text-4xl md:text-5xl font-extrabold text-teal-400 mb-4 animate-fade-in-down">Modification de la Filière</h1>
        <p class="text-lg text-gray-300 mb-8 animate-fade-in-up">Mettez à jour les informations de votre filière</p>
        ...
        <div class="bg-[#112825]/90 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-gray-700 w-full max-w-lg animate-fade-in-up">
            <form action="{{ route('filieres.update', $filiere->id) }}" method="POST" class="space-y-6">
                ...
                <!-- Champ Nom -->
                <div>
                    <input type="text" name="name" value="{{ $filiere->name }}" placeholder="Nom de la filière"
