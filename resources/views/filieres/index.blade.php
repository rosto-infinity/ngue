...
<div class="pt-24 px-6 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto mt-8 relative z-10 text-center">
        ...
        <!-- Grand Titre animé -->
        <h1 id="animated-title" class="text-4xl md:text-5xl font-extrabold text-white mb-4 text-center leading-tight">Gestion des Filières</h1>
        <p class="text-gray-400 text-center mb-12 text-lg">Ajoutez, modifiez et gérez facilement les filières</p>

        <!-- Bouton Ajouter une filière -->
        <div class="flex justify-center mb-8">
            <a href="{{ route('filieres.create') }}" class="inline-block bg-teal-500 hover:bg-teal-600 text-white font-semibold px-8 py-4 rounded-full shadow-lg transition-transform transform hover:scale-105" id="add-filiere-btn">
                + Ajouter une filière
            </a>
        </div>
            ...
            <table class="w-full text-white">
                <thead>
                    <tr class="bg-gradient-to-r from-teal-600 to-teal-800 text-white uppercase text-sm tracking-wider">
                        ...
                        <th class="py-5 px-6 text-left">Nom de la filière</th>
                        <th class="py-5 px-6 text-left">Description</th>
                ...
                <tbody>
                    @foreach ($filieres as $filiere)
                        <tr class="border-b border-gray-700 hover:bg-gray-900/70 transition-all duration-300">
                            ...
                            <td class="py-4 px-6 font-medium text-gray-200">{{ $filiere->name }}</td>
                            <td class="py-4 px-6 text-gray-300">{{ $filiere->description }}</td>
                            <td class="py-4 px-6 flex space-x-2">
                                <a href="{{ route('filieres.edit', $filiere->id) }}" class="bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-full shadow-md transition-transform transform hover:scale-105">
                                    Modifier
                                </a>
                                <form action="{{ route('filieres.destroy', $filiere->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
            ...
            <ul class="space-y-2">
                ...
                <li><a href="#" class="hover:text-teal-400">Ajouter une filière</a></li>
                <li><a href="#" class="hover:text-teal-400">Liste des filières</a></li>
...
const AnimationManager = {
    ...
    animateAddButton() {
        const button = document.getElementById("add-filiere-btn");
        if (button) {
            gsap.from(button, {
                opacity: 0,
                y: 20,
            ...
            console.error("Élément #add-filiere-btn non trouvé.");
        }
    },
        ...
        document.querySelectorAll(".delete-form").forEach(form => {
            form.addEventListener("submit", function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Voulez-vous vraiment supprimer cette filière ? Cette action est irréversible.",
                    icon: "warning",
...
// Appel de l'initialisation des animations et de la gestion des messages
